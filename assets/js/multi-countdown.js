...
$(function () {
    ...
    */
    ...
    let endedClass = "#mis_state"; //optional
    ...
    function dateReplace(extraClass, date, fixTime, initText, zeroPad, $this) {
        ...
        let text = initText;
        let zeroPadArr = [];
        let isend = $('.' + extraClass).attr('data-isend');
        let page = $('.' + extraClass).attr('data-page');
        if (dif[0] < 0 || dif[1] < 0 || dif[2] < 0 || dif[3] < 0) {
            ...
            if (page == 'detail') { //case data-endText attr
                /*
                    ...
                    카운트다운 마감시
                */
                $(endedClass).text(endText);
                $('#ing_div_tag').css('display','none');
                $('#ing_a_tag').css('display','none');
                $('#end_p_tag').css('display','');
                $('#end_a_tag').css('display','');
    ...
    function getFixDate(fixTime) {
        ...
        if (fixTimeDate['Days'] != undefined) { getFixTimeDate += +fixTimeDate['Days'] * 60 * 60 * 24; }
        if (fixTimeDate['Hours'] != undefined) { getFixTimeDate += +fixTimeDate['Hours'] * 60 * 60; }
        if (fixTimeDate['Minutes'] != undefined) { getFixTimeDate += +fixTimeDate['Minutes'] * 60; }
        if (fixTimeDate['Seconds'] != undefined) getFixTimeDate += +fixTimeDate['Seconds'];

        var now = new Date();
        now.setSeconds(now.getSeconds() + getFixTimeDate); // timestamp
