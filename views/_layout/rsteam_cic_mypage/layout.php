<?php
	//페이지별 언어팩 로드
	$this->lang->load('cic_layout_mypage', $this->session->userdata('lang'));
?>
<!DOCTYPE html>
<html lang="ko">
<head>
...
<div id="wrap">
	...
	<div id="header">
		<nav>
			...
			<!--menu-->
			<ul id="menu">
				<li><a href="<?=base_url('About/whitepaper')?>" class="<?=element('menu',$header) == 'about' ? 'on' : '';?>"><?=$this->lang->line('cic_1')?></a></li>
			...
			<div class="box">
				<div class="mypage">
					<a href="<?=base_url('mypage')?>"> <?=$this->lang->line('cic_5')?></a>
				</div>
				<div class="logout">
...
?>
			<div id="sub_menu">
				<ul> 
					<!-- <li><a href="<?php //echo base_url('About/provision')?>" <?php // echo (strpos(uri_string(),'About/provision') !== false) ? 'class="on"': ''?>><?php //$this->lang->line('cic_18')?></a></li> -->
...
<div id="content">
		...
		<div id="mypage">
			<div id="my_left_box">
				<h3><?=$this->lang->line('cic_5')?></h3>
				<div class="my_box">
					<h4><?=$this->lang->line('cic_5')?></h4>
					<div class="user_img">
					<?php if($this->member->group()[0]['mgr_id'] == 1){ ?>
						<img src="<?php echo base_url('assets/images/user.png');?>" alt="<?=$this->lang->line('cic_23')?>" />  <!--//일반유저 프사-->
					<?php }else{ ?>
						<img src="<?php echo base_url('assets/images/s_user.png');?>" alt="<?=$this->lang->line('cic_24')?>" /><!--인플루언서 프사-->
					<?php } ?>
					
					</div> 
					<strong><?=$member_data['mem_nickname']?></strong>
					<!-- <p>
						<?php // echo $this->lang->line('cic_13')?>
						<span><?php //echo number_format($total_super)?></span>
					</p> -->
				</div>
			</div>
