	...
	<div class="box-table">
		<div class="box-table-header">
			<ul class="nav nav-pills">
				<li role="presentation"><a href="<?php echo admin_url($this->pagedir); ?>">퍼포인트 목록</a></li>
				<li role="presentation" class="active"><a href="<?php echo admin_url($this->pagedir . '/cleanlog'); ?>">퍼포인트 로그 정리</a></li>
			</ul>
		</div>
		<?php
		...
		<div class="table-responsive">
			<table class="table table-bordered table-hover table-striped">
				<tbody>
					<tr>
						<td>
							<p>퍼포인트 기능을 사용하시는 경우, 퍼포인트 테이블에 데이터가 급속도로 쌓이게 됩니다.</p>
							<p>퍼포인트 테이블에 데이터가 많이 쌓이게 되면 사이트 속도에 영향을 끼칠 수 있습니다.</p>
							<p>이 페이지는 회원님이 보유하고 계신 퍼포인트의 현재 총 합을 유지한채,</p>
							<p>오래된 퍼포인트 내역을 간단히 정리해 퍼포인트 테이블의 크기를 가볍게 하는데에 목적이 있습니다.</p>
							<p>원하는 기간 이상이 된 퍼포인트 중 한 회원이 10건 이상의 퍼포인트 내역을 가지고 있을 경우, 그 퍼포인트 내역을 1건으로 정리합니다.</p>
							<p>정리를 할 때마다 서버 부하 때문에 100명의 회원 것만 정리합니다. 회원이 많을 경우, 정리하기를 여러번 실행하시면 됩니다.</p>
							<?php
							$attributes = array('class' => 'form-horizontal', 'name' => 'fadminwrite', 'id' => 'fadminwrite');
							echo form_open(current_full_url(), $attributes);
							?>
								<input type="number" class="form-control" name="day" value="<?php echo set_value('day', 30); ?>" /> 일 이상된 퍼포인트 내역을 검색합니다.
