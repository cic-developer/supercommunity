...
class IOS extends CB_Controller
{
	 ...
	 * 모델을 로딩합니다
	 */
	protected $models = array('Board', 'RS_media', 'RS_main_config', 'Post', 'Post_meta', 'Post_extra_vars');
	...
	public function lists($brd_key = '')
	{
		...
		$layoutconfig = array(
			...
			'skin' => $list_skin_file,
			'layout_dir' => '/IOS',
			'mobile_layout_dir' => 'IOS',
			'use_sidebar' => $use_sidebar,
			'use_mobile_sidebar' => $use_mobile_sidebar,
			'skin_dir' => '/cic_ios',
			'mobile_skin_dir' => '/cic_ios',
			'page_title' => $page_title,
			'meta_description' => $meta_description,
			'meta_keywords' => $meta_keywords,
			'meta_author' => $meta_author,
	...
	public function post($post_id = 0, $print = false)
	{
		...
		$view['view']['post_url'] = $post_url = ios_post_url('IOS',element('brd_key', $board), $post_id);
		...
		if ($skeyword) {
			$view['view']['list_url'] = base_url('IOS/lists/'.element('brd_key', $board));
			$view['view']['search_list_url'] = base_url('IOS/lists/'.element('brd_key', $board) . '?' . $param->output());
		} else {
			$view['view']['list_url'] = base_url('IOS/lists/'.element('brd_key', $board) . '?' . $param->output());
		...
		if ($use_prev_next) {
			...
			if (element('post_id', $next_post)) {
				$view['view']['next_post']['url'] = ios_post_url('IOS',element('brd_key', $board), element('post_id', $next_post)) . '?' . $param->output();
			}
			...
			if (element('post_id', $prev_post)) {
				$view['view']['prev_post']['url'] = ios_post_url('IOS',element('brd_key', $board), element('post_id', $prev_post)) . '?' . $param->output();
			}
		}
		...
		if ($print === false) {
			...
			$view['view']['short_url'] = $view['view']['canonical'] = ios_post_url('IOS',element('brd_key', $board), $post_id);
			...
			if(element('use_bitly', $board)) {
				if(element('bitly_url', element('meta', $post))) {
					...
					$bitlyparams['longUrl'] = ios_post_url('IOS',element('brd_key', $board), $post_id);
	...
	public function _get_list($brd_key, $from_view = '')
	{
		...
		if ($noticeresult) {
			foreach ($noticeresult as $key => $val) {
				...
				$noticeresult[$key]['post_url'] = ios_post_url('IOS',$notice_brd_key, element('post_id', $val));
		...
		if (element('list', $result)) {
			foreach (element('list', $result) as $key => $val) {
				$result['list'][$key]['post_url'] = ios_post_url('IOS',element('brd_key', $board), element('post_id', $val));
		...
		$config['base_url'] = base_url('IOS/lists/'.$brd_key) . '?' . $param->replace('page');
		...
		if ($skeyword) {
			...
			$view['list_url'] = base_url('IOS/lists/'.element('brd_key', $board));
			$view['search_list_url'] = base_url('IOS/lists/'.element('brd_key', $board) . '?' . $param->output());
		} else {
			$view['list_url'] = base_url('IOS/lists/'.element('brd_key', $board));
