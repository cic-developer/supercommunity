...
class Loginlog extends CB_Controller
{
	...
	{
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_admin_member_badlogin_index';
		$this->load->event($eventname);
		 ...
		 * 게시판 목록에 필요한 정보를 가져옵니다.
		 */
		$this->{$this->modelname}->allow_search_field = array('member_login_log.mll_id', 'member_login_log.mll_userid', 'member_login_log.mll_success', 'member_login_log.member_login_log.mem_id', 'member_login_log.mll_datetime', 'member_login_log.mll_ip', 'member_login_log.mll_reason', 'mll_log_agent', 'member_login_log.mll_url', 'member_login_log.mll_referer'); // 검색이 가능한 필드
		$this->{$this->modelname}->search_field_equal = array('member_login_log.mll_id', 'member_login_log.mem_id'); // 검색중 like 가 아닌 = 검색을 하는 필드
		$this->{$this->modelname}->allow_order_field = array('member_login_log.mll_id'); // 정렬이 가능한 필드
		$result = $this->{$this->modelname}
			->get_bad_list($per_page, $offset, '', '', $findex, $forder, $sfield, $skeyword);
		 ...
		 * 페이지네이션을 생성합니다
		 */
		$config['base_url'] = admin_url($this->pagedir) . '/badlogin?' . $param->replace('page');
		$config['total_rows'] = $result['total_rows'];
		$config['per_page'] = $per_page;
		$this->pagination->initialize($config);
		$view['view']['paging'] = $this->pagination->create_links();
		 ...
		 * 쓰기 주소, 삭제 주소등 필요한 주소를 구합니다
		 */
		$search_option = array('member_login_log.mll_datetime' => '날짜', 'member_login_log.mll_ip' => 'IP', 'member_login_log.mll_reason' => '이유');
		$view['view']['skeyword'] = ($sfield && array_key_exists($sfield, $search_option)) ? $skeyword : '';
		$view['view']['search_option'] = search_option($search_option, $sfield);
		$view['view']['listall_url'] = admin_url($this->pagedir).'/badlogin';
		$view['view']['list_delete_url'] = admin_url($this->pagedir . '/listdelete/?' . $param->output());
		 ...
		 * 어드민 레이아웃을 정의합니다
		 */
		$layoutconfig = array('layout' => 'layout', 'skin' => 'badlogin');
		$view['layout'] = $this->managelayout->admin($layoutconfig, $this->cbconfig->get_device_view_type());
		$this->data = $view;
		$this->layout = element('layout_skin_file', element('layout', $view));
		$this->view = element('view_skin_file', element('layout', $view));
