...
$lang[1] = 'To see more details';  // 더보기 버튼
$lang[2] = '<h4>1. Introducation</h4>
<ul class="text_g">
    <li class="sub_title">
        Current Address of Blockchain Distributed Principal Technology 
    </li>
    <li>
        <p>
...
<ul class="text_g">
    <li class="sub_title">
        Application of Blockchain and Digital Assets to Business Models 
    </li>
    <li>
        <p>
...
<ul class="text_g">
    <li>
        <p>Every individual can forward an ad message from the PER Token platform to someone online and receive a token reward.</pp
    </li>
    <li class="img">
    ...
    <li>
        <p> The ecosystem of the initial PER token consists of:</pp   
    </li>
    <li class="img">
        <img src="https://dev.percommunity.com/assets/images/white002_en.jpg" alt="Figure02">
    </li>
    <li class="sub_title">
        Forwarding System Model with Blockchain
    </li>
    <li>
        Ppp All PER tokens paid by advertisers as advertising expenses are distributed to Per Friends, which remains as a record on the block chain to check the execution of advertising funds.</pp   
    </li>
    <li class="img">
    ...
    <li class="sub_title">
        SUPER COMMUNITY - Platform participant compensation diagram 
    </li>
    <li>
        <p> PER Per Friends advertising activity compensation is done in the compensation collection, which can be seen on the block chain by anyone. The wallet must always be deposited with 400 million PER tokens, which are covered by SUPER COMMUNITY&#39;s purchase on the exchange in a compensatory glove.</pp   
    </li>
    <li class="img">
    ...
    <li>
        <p>In the future, the role of SUPER COMMUNITY will be reduced and the autonomous ecosystem will be established.</pp
    </li>
    <li class="img">
    ...
    <li>
        <p> Future tokens are more convenient to use, making it easier for non-coin traders to participate in the ecosystem.</pp
    </li>
    <li class="img">
    ...
    <p>Cash or use tokens without going through the exchange, eventually becoming a rewarding advertising platform for anyone to participate in, and tokens are used in the block chain.
        ...
        Rewards for advertising activities can be easily obtained oldspan class="p_bold"></span>, and this token is immediately cashed in or as a good participation reward with many direct uses. 
    ...
    <li class="sub_title">
        PER Token&#39;s main marketing target platform
    </li>
    <li>
        <ul class="list">
            <li>
...
<ul>'; 
$lang[6] = '<h4>5. Token issue</h4><ul class="text_g"> 
    <li><p>
        Token sales are not conducted considering the side effects and irrationalities of ICO. The advisor reward token is 2 billion. This is in accordance with our token lock-up policy.
    ...
    <li class="s_img"> <img src="https://dev.percommunity.com/assets/images/white011_en.jpg" alt="Figure11"> </li> 
    <li> Project protocol : Klaytn / ERC 20<br> Currency Accepted : Klaytn </li>
    <li class="sub_title"><p>
        Non-current token locked up as token. It cannot be distributed in practice, but you need to have a lot of compensation base points to get a lot of compensation to get a lot of course.
    ...
    <li class="img"> <img src="https://dev.percommunity.com/assets/images/white012_en.jpg" alt="Figure12"> </li> 
    <li class="sub_title"> 
        Project protocol : Klaytn / ERC 20<br> Currency Accepted : Klayt
    </li>
    <li><p>
        (https://docs.klaytn.com/api/platform). This transaction is sent to the Consensus Node (CN) and stored in a block.
        The CN checks the validity of the received transactions. Valid transactions are stored or discarded in the transaction pool.
        The CN selects a viable transaction within the current block in the transaction pool and executes one each. 
        To execute a transaction, the sender must pay a partial KLAY fee for the transaction. 
        The transaction costs paid in clay (KLAY) are calculated based on multiplier effects such as gas and gas costs. 
        Gas is the basic unit of calculation. All activities running on the Claytn node consume a predefined amount of gas. 
        The exact amount of clay (KLAY) required for a transaction is calculated by the formula specified in the transaction cost. (https://docs.klaytn.com/klaytn/design/computation/exec_model).
        If the sender submits a transaction when there is insufficient gas, the 22 transactions may fail, and even if the sender&#39;s account balance is insufficient, the transaction may fail.             
        If the transaction runs successfully, it will be included in the current block. The CN collects transactions until the block gas limit or block time limit is reached, and the CN creates blocks from the transaction. At this stage, you must populate several fields in the block.
        For example, you need to calculate the hash value, amount received, and status of the transaction. After completing all required fields, the CN generates a block hash.             
        ...
        The important thing is that these transactions require payment for execution. The sender&#39;s account balance decreases depending on the transaction cost model after the transaction is stored in the block. After a certain period of time, the transaction appears in the block and confirms that the accompanying status has been agreed. At this point, smart contracts exist in Clayton Blockchain. <br><br> Smart Contract Execution - The functions of a smart contract can be invoked and executed by sending transactions to a smart contract or by invoking functions to a node. If you send a transaction to invoke a function, the function is executed by processing the transaction. This includes the cost of sending transactions within the clay (KLAY), and calls are recorded in the block chain forever. The return value of this generated call is the hash of the transaction. When the feature is applied, it runs internally in the Clayton (Klaytn) virtual machine (KLVM), and the call returns the function&#39;s return value. The internal status of the contract cannot be modified because these generated calls are not recorded in the block chain.             
        This type of call is called a constant function call. This generated call does not require a clay (KLAY). Constant function calls should be used when return values are applicable, and transactions should be used when side effects of contract status are applicable.             
        Clayton (Klaytn) Token and Transaction Costs - Clay (KLAY) is a major internal transactionable crypto of Clayton (KLAY) and is used to create or execute smart contracts or pay transaction costs for Clay (KLAY) remittances. Clay (KLAY) is primarily a fuel requirement for Clayton (Klaytn) network operations. This is a consensual node (CN) for executing the requested activity in the form of payment made by platform users. In other words, clay (KLAY) is an incentive and ensures developers use high-quality applications and maintain the network&#39;s health line (which requires higher fees for wasteful code). (The Clayton (Klaytn) node is compensated for the resources it contributes.) Transaction costs within Clayton (Klaytn) can be calculated in a variety of ways, depending on the execution environment in which the transaction runs.             
        For example, the current transaction cost for Clayton (Klaytn) virtual machines (KLVMs) can be calculated as follows:             
        : Transaction cost : = F (total gas used) x (unit price) ● F() can be a step-by-step pricing function. This feature is not yet confirmed and may change with further development of Clayton (Klaytn). Clayton (Klaytn) For a testnet, F() is an anisotropic function, and transaction costs can be simply calculated as (total gas) X (unit price). ● Total gas used can be calculated by KLVM according to the gas cost and the unique gas cost of the OP Code. These calculated transaction costs are deducted from the balance of the sender&#39;s or corporate account according to the transaction. For more information on KLAY tokens and transaction costs, please check out the Claytn documentation on the website.             
        (https://docs.klaytn.com/klaytn/design/computation/exec_model). <br><br> service chain - The service chain in Clayton (Klaytn) is an independent secondary block chain that is tailored to individual Bapps that require special node configurations, custom security ratings, or unusually high throughput that make the deployment of the Bapp in the Claytn main chain inconvenient or uneconomical. There is a sufficiently decentralized scaling solution, but try or get out
    </p></li>';
$lang[7] = '<h4>6. Lock-Up Policy</h4>
...
<ul class="text_g">
    ...
    <li class="s_img"
        <img src="https://dev.percommunity.com/assets/images/white015_en.jpg" alt="Figure 15">
    </li>
    <li>
        <ul class="list">
            <li>
